...
def sim_insertions(numBuckets, numInsertions):                              #  走一遍，如果有 collision，就return False。一个collision都没有，就return True
    '''
...
def observe_prob(numBuckets, numInsertions, numTrials):                    #  走 numTrials 遍，记录 [True, False, False] 进入 probs，sum的时候一次True算一次1
    '''                                                                     #  最后return的是失败的概率，会接近 终极计算的值
    Given the number of buckets and the number of items to insert, 
...
def classroom(low, high, rate):           #  自己写的
    middle = (high+low)/2
    if low == middle:                    #  这个地方如果写 low == high 则进入死循环，因为 (56,57) 永远不会 low == high
        return low
    elif collision_prob(365, middle)>rate:
        return classroom(low,middle,rate)
    else:
        return classroom(middle,high,rate)
low=0
high=250
rate=0.99      
print 'result: ', classroom(low, high,rate)
print 'test of result  ',collision_prob(365,classroom(low, high,rate))
print 'test of result+1',collision_prob(365,classroom(low, high,rate)+1)
