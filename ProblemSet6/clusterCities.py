...
class Point(object):
    ...
    def getAttrs(self):
        return self.attrs
    def distance(self, other):     ####  默认求的是直线距离
...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for point1 in self.points:
            for point2 in other.points:
                if shortest == None or point1.distance(point2) < shortest:
        ...
        for point1 in self.points:
            for point2 in other.points:
                if greatest == None or point1.distance(point2) > greatest:
                    greatest = point1.distance(point2)
        return greatest
    ...
    def averageLinkageDist(self, other):
        ...
        for point1 in self.points:
            for point2 in other.points:
                alls += point1.distance(point2)
        return float(alls)/float(len(self.points))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        new = list(i for i in c1.points)+list(n for n in c2.points)
        self.members.remove(c1)
        self.members.remove(c2)
        self.members.append(Cluster(new, City))
    ...
    def findClosest(self, linkage):
        ...
        for c1 in self.members:
            for c2 in self.members:
                if c1 != c2:
                    if distance == None or linkage(c1,c2) < distance:           ##  这个地方不能写 c1.linkage(c2)， 没有linkage这个命令!
...
def readCityData(fName, scale = False):
    ...
    dataFile = open('E:\Edx\MIT_Python_2\ProblemSet6\cityTemps.txt', 'r')
    numFeatures = 0
